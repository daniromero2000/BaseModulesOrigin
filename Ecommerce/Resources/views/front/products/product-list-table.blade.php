                ...
                <div class="row">
                        ...
                        <form action="{{ route('cart.update', $cartItem->rowId) }}" class="form-inline" method="post">
                            ...
                            <div class="input-group">
                                ...
                                <span class="input-group-btn"><button
                                        class="btn btn-default btn-sm">Actualizar</button></span>
                            </div>
                        </form>
                    </div>
                    <div class="col-lg-1 col-md-1 col-sm-8 col-xs-4">
                        <form action="{{ route('cart.destroy', $cartItem->rowId) }}" method="post">
                            ...
                            <button onclick="return confirm('¿Estás Seguro?')" class="btn btn-danger btn-sm"><i
                                    class="fa fa-times"></i></button>
                        </form>
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                        <span class="hidden-lg hidden-md"><small>Iva: </span>
                        {{config('cart.currency')}} {{ number_format(($cartItem->qty*$cartItem->tax), 0) }}</small>
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                        <span class="hidden-lg hidden-md"><small>Subtotal: </span>
                        {{config('cart.currency')}} {{ number_format($cartItem->price, 0) }}</small>
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                        <span class="hidden-lg hidden-md"><small>Total: </span>
                        {{config('cart.currency')}} {{ number_format(($cartItem->qty*$cartItem->price), 0) }}</small>
                    </div>
                </div>
            </div>
        </div>

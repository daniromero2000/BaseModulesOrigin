...
<form action="{{ route('payu.store') }}" method="POST">
    @csrf
    <div class="row py-4">
        <div class="col-12">
            <div class="form-group">
                <label class="text-sm " for="">Número de tarjeta</label>
                <input type="text" required validation-pattern="number" name="CREDIT_CARD_NUMBER" class="form-control"
                    id="" aria-describedby="textHelp">
            </div>
                    ...
                    <div class="form-check container-card">
                        ...
                        <label class="form-check-label" for="creditCards{{$item->id}}">
                            <img src="{{asset($item->icon)}}" alt="{{$item->icon}}"
                                style=" max-width: 37px; border-radius: 5px; "></label>
                    </div>
                </div>
                @endforeach
            </div>

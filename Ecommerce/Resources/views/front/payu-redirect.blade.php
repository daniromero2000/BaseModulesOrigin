...
<div class="container product-in-cart-list">
    <div class="row">
        ...
        <div class="col-md-12">
            <form action="{{ route('payu.store') }}" class="form-horizontal" method="post">
                {{ csrf_field() }}
                <div class="col-md-6">
                    <h3>Review</h3>
                    <hr>
                    <ul class="list-unstyled">
                        ...
                        <li>Iva: {{ config('cart.currency_symbol') }} {{ $tax }}</li>
                        <li>Total Envío: {{ config('cart.currency_symbol') }} {{ $shipping }}</li>
                        <li>Total: {{ config('cart.currency_symbol') }} {{ $total }}</li>
                    </ul>
                </div>
                <div class="col-md-6">
                    <div class="box-body">
                        <h3>{{ config('payu.bank_name') }}</h3>
                        <hr>
                        ...
                        <hr>
                        <div class="btn-group">
                            <a href="{{ route('checkout.index') }}" class="btn btn-default">Regresar</a>
                            <button onclick="return confirm('¿Estás Seguro?')" class="btn btn-primary">Pagar ahora con transferencia Bancaria</button>

    ...
    <div class="box crud-box" style="box-shadow: 0px 2px 25px rgba(0, 0, 0, .25);">
        <form action="{{ route('admin.customers.update', $customer->id) }}" method="post" class="form">
            <div class="box-body">
                ...
                <div class="form-group">
                    <label for="customer_channel_id">Estado <span class="text-danger">*</span></label>
                    <div class="input-group">
                        <div class="input-group-addon">
                            <i class="fa fa-check"></i>
                        </div>
                        <select name="customer_channel_id" id="customer_channel_id" class="form-control select2">
                            ...
                            <option @if($customer_channel->id == $lead) selected="selected" @endif
                                value="{{ $customer_channel->id }}">{{ $customer_channel->lead }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>

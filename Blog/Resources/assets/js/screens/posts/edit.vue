...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        body: "",
        published: false,
        markdown: { null: null, markdown: true, rich: false }[
  ...
  methods: {
    ...
    watchBodyChangesAndSave() {
      setTimeout(() => {
        this.postBodyWatcher = this.$watch(
          "form.body",
          _.debounce(() => this.save(), 1000),
          { deep: true }
        );
      }, 1000);
    },
    ...
    deletePost() {
      this.alertConfirm("Are you sure you want to delete this post?", () => {
        ...
        this.http()
          .delete("/api/posts/" + this.id, this.form)
          .then((response) => {
    ...
    publishPost() {
      ...
      this.notifySuccess("Publicación publicada!", 2000);
    },
    ...
    save() {
      ...
      if (
        this.form.title != "Draft" &&
        (!this.form.slug || this.form.slug.startsWith("draft-"))
      ) {
...
<template>
  <div>
    <page-header>
      <div slot="left-side">
        <div v-if="ready && entry">
          <span class="font-semibold" v-if="!status && form.published"
            >Publicada</span
          >
          <span class="font-semibold" v-if="!status && !form.published"
            >Draft</span
      ...
      <div class="flex items-center" v-if="ready && entry" slot="right-side">
        <button
          class="py-1 px-2 btn-primary text-white border-0 text-sm mr-6"
        ...
        <div class="card my-5">
          <div class="card-body pt-5">
            <div class="lg:w-3/4 mx-auto">
              ...
              <editor
                v-if="form.markdown == false"
                :post-id="id"
                v-model="form.body"
              ></editor>
              <markdown-editor v-model="form.body" v-if="form.markdown == true">
              </markdown-editor>
            </div>
          </div>
        </div>
    ...
    <modal v-if="settingsModalShown" @close="closeSettingsModal">
      ...
      <div class="mt-10">
        <button
          class="btn-sm btn-primary text-white border-0"
    ...
    <modal v-if="publishingModalShown" @close="publishingModalShown = false">
      <div
        ...
        v-if="
          form.title == 'Draft' || !form.slug || form.slug.startsWith('draft-')
        "
      >
        Asegúrate de que tu publicación tenga un título y un slug amigables.
      </div>

      <div class="input-group pt-0">
        <label class="input-label">Fecha de publicación (M/D/Y H:M) UTC</label>
      ...
      <div class="mt-10">
        ...
        <button
          class="btn-sm ml-1 btn-light text-white border-0"
          @click="unpublishPost"
          v-if="form.published"
          v-loading="status"
        >
        ...
        <button
          class="btn-sm ml-1 btn-light text-white border-0"
          @click="publishingModalShown = false"
        >
          Cancelar
        </button>
      </div>
